...
for variant in variants:
    rare_var_and_exp_change        = df[(df[variant] != 0) & (~(df['Alt. splicing'].isna()))].shape[0]
    rare_var_and_not_exp_change    = df[(df[variant] != 0) & (df['Alt. splicing'].isna())].shape[0]
    no_rare_var_and_exp_change     = df[(df[variant] == 0) & (~(df['Alt. splicing'].isna()))].shape[0]
    no_rare_var_and_not_exp_change = df[(df[variant] == 0) & (df['Alt. splicing'].isna())].shape[0]
...
stats.to_csv('alt_splicing_log_odds.tsv', sep='\t')