...
brainspan_filename  = '../data/brainspan_preferential_tissue_expression.tsv'
...
te = pd.read_csv(brainspan_filename, sep='\t')
te = te.set_index('ensembl_gene_id', drop=True)
...
stats.to_csv('output/brainspan_enrichment_of_de_genes.tsv', sep='\t', index=False)